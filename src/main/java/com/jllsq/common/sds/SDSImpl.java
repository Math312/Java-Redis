...
public class SDSImpl implements SDS{
    ...
    public SDSImpl sdsDup() {
        SDSImpl sdsImpl = new SDSImpl();
        sdsImpl.length = this.length;
        sdsImpl.used = this.used;
        sdsImpl.content = new byte[sdsImpl.length];
        System.arraycopy(this.content, 0, sdsImpl.content, 0, this.used);
        return sdsImpl;
    }
